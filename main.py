...
def register():
    ...
    if register_form.validate_on_submit():
        if User.query.filter_by(email=register_form.email.data).first():
            ...
            new_user = User(email=register_form.email.data, password=hash_password,
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if not user_to_login:
            ...
            return redirect(url_for('login'))
        elif not check_password_hash(user_to_login.password, login_form.password.data):
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            new_comment = Comment(parent_post=requested_post, comment_author=current_user,
                                  text=form.comment.data)
            db.session.add(new_comment)
            db.session.commit()
            return redirect(url_for('show_post', post_id=post_id))
        else:
            flash("You need to log in or to register to comment")
            return redirect(url_for('login'))
